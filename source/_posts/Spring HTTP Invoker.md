...
Spring HTTP invoker 是 spring框架中的一个远程调用模型，执行基于HTTP的远程调用（意味着可以通过防火墙），并使用Java的序列化机制在网络间传递对象。这需要在远端和本地都使用Spring才行，客户端可以很轻松地像调用本地对象一样调用远程服务器上地对象，这有点类似于webservice，但又不同于webservice，区别如下：

| WebService                                  | Http Invoker                     |
| ------------------------------------------- | -------------------------------- |
| 跨平台                                      | 只支持java语言                   |
| 支持SOAP，提供wsdl                          | 不支持                           |
| 结构庞大，依赖特定的webservice实现，如xfire | 结构简单，只依赖于spring框架本身 |

说明：

1 服务器端：通过HTTP Invoker服务将服务接口的某个实现类提供远程服务
