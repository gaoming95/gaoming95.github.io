...
//连接服务器端的地址和端口号
        ServerAddress serverAddress = new ServerAddress("172.16.39.99", 27017);
//project数据库的用户名为:proj 密码：proj
        char[] password = "test".toCharArray();
//创建 用户名和密码认证
        MongoCredential mongoCredential = MongoCredential.createCredential("test", "business_mgk", password);
...
//连接MongoDB的一个数据库，需要用户名、密码、数据库、服务器端的IP和端口号
        ...
        MongoDatabase mongoDatabase = mongoClient.getDatabase("business_mgk");
